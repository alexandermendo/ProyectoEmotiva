...
});// Importa el módulo multer para manejar form-data
const upload = multer({ storage: storage }); // Crea una instancia de multer
...
router.post('/addSliderItem', upload.single('image'), async (req, res, next) => {
  try {
    ...
    if (!title || !subtitle || !description || !fotoFilePath) {
