...
export const LifeStyleDash = () => {
  ...
  const handleSaveUpdate = async () => {
    try {
      ...
      const response = await fetch(`${url}/lifestyle/${lifeEditing._id}`, { method: 'PUT', body: formData });
  ...
  const handleDelete = async (_id) => {
    try {
      const response = await fetch(`${url}/lifestyle/${_id}`, {
        method: 'DELETE',
      });
            ...
            <div className="col-md-12">
              <form>
                ...
                <div className="slide d-flex justify-content-end">
                  <button type="button" onClick={handleCancelUpdate} className="btn-cancel-life">
                    Cancelar
                  </button>
                  <button type="button" onClick={handleSaveUpdate} className="btn-update-life">
                    Guardar Cambios
                  </button>
                </div>
              </form>
